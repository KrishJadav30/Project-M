...
import { Sparkles, Heart, MessageCircleHeart, HeartHandshake, Infinity as InfinityIcon } from 'lucide-react';
...
export const Timeline: React.FC = () => {
  const getIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case 'MessageCircleHeart':
        return <MessageCircleHeart className="w-5 h-5 text-pink-500" />;
      case 'HeartHandshake':
        return <HeartHandshake className="w-5 h-5 text-rose-600" />;
      case 'Infinity':
        return <InfinityIcon className="w-5 h-5 text-red-500" />;
      default:
  ...
  return (
    <section id="story" className="py-20 px-4 sm:px-6 relative">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <p className="mt-3 text-sm sm:text-base text-rose-900/70 max-w-lg mx-auto">
            A small glimpse into the moments that made me fall hopelessly in love with you.
          </p>
        </div>
            ...
            {loveData.milestones.map((item, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute left-6 md:left-1/2 -translate-x-1/2 flex items-center justify-center w-11 h-11 rounded-full bg-white border-2 border-rose-300 shadow-md shadow-rose-200/60 z-10">
                    {getIcon(item.icon)}
                  </div>
                  ...
                  <div className="w-full md:w-1/2 pl-14 md:pl-0">
                    <div className="glass-card rounded-3xl p-6 sm:p-7 border border-rose-200/80 bg-white/85 shadow-lg shadow-rose-100/70 hover:shadow-xl hover:border-rose-300 transition-all duration-300">
                      <div className="flex items-center justify-between mb-3">
                        <span className="text-xs font-bold uppercase tracking-wider text-rose-500 bg-rose-50 px-2.5 py-1 rounded-full border border-rose-100">
                          {item.date}
                        </span>
